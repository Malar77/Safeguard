...
import CounselorDashboardScreen from "./src/screens/counselor/CounselorDashboardScreen";
import CounselorAppointmentsScreen from "./src/screens/counselor/CounselorAppointmentsScreen";
...
function stateHasRoute(state, targetName) {
  ...
  for (const route of state.routes) {
    ...
    if (route?.state && stateHasRoute(route.state, targetName)) return true;
  }
  return false;
}
...
function AppNavigator() {
  const { user, loading, isParent, isCounselor } = useAuth();
  ...
  return (
    ...
    >
      {!user ? (
        ...
        <>
          {isCounselor ? (
            <Stack.Screen name="CounselorDashboard" component={CounselorDashboardScreen} options={{ headerShown: false }} />
          ) : (
            <Stack.Screen name="Main" component={isParent ? ParentTabs : MainTabs} options={{ headerShown: false }} />
          )}
          {isCounselor && (
            <Stack.Screen name="CounselorAppointments" component={CounselorAppointmentsScreen} options={{ title: "Appointments" }} />
          )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onBackPress = () => {
      ...
      const homeTab = stateHasRoute(rootState, "ParentHome") ? "ParentHome" : "Dashboard";

      if (["SOS", "Report", "Profile"].includes(current)) {
        navigationRef.navigate("Main", { screen: homeTab });
        return true;
      }

      // Let Android close the app only from home screens.
      if (["Dashboard", "ParentHome", "CounselorDashboard"].includes(current)) {
        return false;
      }
