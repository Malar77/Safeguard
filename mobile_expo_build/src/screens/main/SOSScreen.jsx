...
export default function SOSScreen() {
  ...
  const triggerSOS = async () => {
    ...
    try {
      ...
      let selfie_data = selfieBase64 ? `data:image/jpeg;base64,${selfieBase64}` : null;
      if (!selfie_data && cameraRef.current) {
        const photo = await cameraRef.current.takePictureAsync({ base64: true, quality: 0.5 });
        setSelfieUri(photo.uri);
        setSelfieBase64(photo.base64);
        selfie_data = photo.base64 ? `data:image/jpeg;base64,${photo.base64}` : null;
      }
      ...
      const payload = {
        ...
        address,
        selfie_data: selfie_data || undefined,
      };
      ...
      try {
        await familyAPI.sendAlert({
          ...
          selfie_data: payload.selfie_data,
        });
      } catch {
        // Guardian push failure should not block SOS creation.
      }

      Alert.alert("SOS Activated", "Emergency alert sent with automatic recording.");
      await loadAlerts();
    } catch (err) {
