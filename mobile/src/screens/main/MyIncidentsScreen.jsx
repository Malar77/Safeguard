            ...
            <TouchableOpacity key={inc.id} style={styles.card} onPress={() => setExpanded(isOpen ? null : inc.id)} activeOpacity={0.85}>
              ...
              {isOpen && (
                <View style={styles.expanded}>
                  ...
                  {inc.evidence_url && (
                    <TouchableOpacity onPress={() => Linking.openURL(inc.evidence_url)}>
                      <Text style={styles.evidenceLink}>View attached media</Text>
                    </TouchableOpacity>
                  )}
                  {inc.admin_notes && (
                    <View style={styles.noteBox}>
                      <Text style={styles.noteLabel}>📌 Admin Note</Text>
                      <Text style={styles.noteText}>{inc.admin_notes}</Text>
                    </View>
                  )}
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  evidenceLink: { marginTop: 10, fontSize: 13, color: COLORS.primary, fontWeight: "700" },
